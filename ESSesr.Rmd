...
```{r nafo4xmap, echo = FALSE, out.width="75%", fig.align="center", fig.cap="Eastern Scotian Shelf"}
#integrate new map
...
The Ecosystem objectives for the Eastern Scotian Shelf (ESS) outline the ecosystem context and consideration for this management area. 
...
bb4vn <- "bb_sstSatellite_4Vn.Rdata"
  ...
  psstSatellite4Vn<-MAReco::plot_temperature_anomaly(sstSatellite4vn, year, normalizedAnomaly, mean_value4vn, sd_value4vn, ff, bb, y_label = "NAFO 4Vn SST Anomaly" ) 
...
bb4Vs <- "bb_sstSatellite_4Vs.Rdata"
  ...
  psstSatellite4Vs<-MAReco::plot_temperature_anomaly(sstSatellite4Vs, year, normalizedAnomaly, mean_value4Vs, sd_value4Vs, ff, bb, y_label = "NAFO 4Vs SST Anomaly" )   
...
bb4W <- "bb_sstSatellite_4W.Rdata"
  ...
  psstSatellite4W<-MAReco::plot_temperature_anomaly(sstSatellite4W, year, normalizedAnomaly, mean_value4W, sd_value4W, ff, bb, y_label = "NAFO 4W SST Anomaly" )   
  
  (psstSatellite4Vn/psstSatellite4Vs/psstSatellite4W)
...
bb4vn <- "bb_bottomt_4Vn.Rdata"
  ...
  pbottomt4Vn<-MAReco::plot_temperature_anomaly(bottomt4vn, year, normalizedAnomaly, mean_value4vn, sd_value4vn, ff, bb, y_label = "NAFO 4Vn Bottom Temp Anomaly" ) 
...
bb4Vs <- "bb_bottomt_4Vs.Rdata"
  ...
  pbottomt4Vs<-MAReco::plot_temperature_anomaly(bottomt4Vs, year, normalizedAnomaly, mean_value4Vs, sd_value4Vs, ff, bb, y_label = "NAFO 4Vs Bottom Temp Anomaly" )   
...
bb4W <- "bb_bottomt_4W.Rdata"
  ...
  pbottomt4W<-MAReco::plot_temperature_anomaly(bottomt4W, year, normalizedAnomaly, mean_value4W, sd_value4W, ff, bb, y_label = "NAFO 4W Bottom Temp Anomaly" )   
  
  (pbottomt4Vn/pbottomt4Vs/pbottomt4W)
...
pbt_4Vn<-plot_indicator(BottomT_ind_4Vn, YEAR, Bottom_Temp, y_label="NAFO 4Vn Bottom Temperature") 

pbt_4Vs<-plot_indicator(BottomT_ind_4Vs, YEAR, Bottom_Temp, y_label="NAFO 4Vs Bottom Temperature")

pbt_4W<-plot_indicator(BottomT_ind_4W, YEAR, Bottom_Temp, y_label="NAFO 4W Bottom Temperature") 
...
- However, formerly commercial but now depleted spp will be at much greater risk due to their low population sizes  ([Boyce et al. 2024](https://www.nature.com/articles/s44183-024-00067-5), see Fig 5 for list).

- By 2050, a valuable species such as lobster is not expected to be vulnerable in Lobster Fishing Area (LFA) 34 and offshore in 4X, but does show increased vulnerability in the Bay of Fundy ([Greenan et al. 2019](https://www.frontiersin.org/journals/marine-science/articles/10.3389/fmars.2019.00579/full)). 
...
```{r ,fig.align="center", echo = FALSE,fig.width = 10, fig.height=7,fig.pos='H', fig.cap="Fishing pressure from 1970-2020 calculated from DFO Summer Research Vessel survey. Data have been corrected for catchability with the exception of invertebrates. Dotted lines denote long-term means +/- 1 sd from the available timeseries. Purple, shaded areas represent the latest 5 years of available, robust data."}
...
```{r, fig.align="center", echo = FALSE,fig.width = 10, fig.height=7, fig.pos='H',fig.cap ="Commercial landings collected from NAFO, ZIF and MARFIS databases for species categories in NAFO 4X. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data."}
...
land_inv_4x<-eco_indicators %>%
  filter(ID=="ESS")%>%
        select(YEAR, landings_INVERTEBRATES.L)
  
    land_for_4x<-eco_indicators %>%
  filter(ID=="ESS" )%>%
        select(YEAR, landings_FORAGE.L)
    
      land_gad_4x<-eco_indicators %>%
  filter(ID=="ESS" )%>%
...
- Overall commercial fisheries landings are decreasing over the last 5 years. 

- Commercial fisheries landings for forage fish and gadoids have generally been below the long-term average for the last 5 years. Invertebrates landings have been increasing over time, but have been in declining over the last 5 years (but still above it's long-term mean). These trends coincide with shifting availability of species in the area. 
...
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=10,fig.pos='H', fig.cap="Nitrate 0-50m, Nitrate 50-150, from Louisburg Line (LL), Chlorophyll (surface) from the Eastern Scotian Shelf. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data. "}
dat.dir<-here::here("data")
...
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=10,fig.pos='H', fig.cap=" and Bloom statistics,for the Eestern Scotian Shelf. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data. "}
azmpdata::RemoteSensing_Annual_Broadscale
...
```
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=10,fig.pos='H', fig.cap=" C.finmarchicus, and meso zooplankton biomass (dry weight), from NAFO 4X. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data. "}
...
pcf4v<-plot_indicator(CF_ind_4V, YEAR, Calanus_finmarchicus, y_label="NAFO 4V Calanus finmarchicus")
pcf4w<-plot_indicator(CF_ind_4W, YEAR, Calanus_finmarchicus, y_label="NAFO 4W Calanus finmarchicus")
...
```{r fig.align="center", echo = FALSE,fig.width = 5, fig.height=4,fig.pos='H', fig.cap="Large fish indicator from RV survey data that has been corrected for catchability. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data."}
dat.dir<-here::here("data")
eco_indicators<- "eco_indicators_esswss.Rdata"
  load(file.path(dat.dir, eco_indicators))
...
```
- LFI in the NAFO 4X area has varied over time. Latest three years indicate that LFI is near the long-term mean and indicate that structure and functioning of the NAFO 4X ecosystem is stable with a current balance in terms of proportion of large fish to small fish in the ecological community. 
...
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=7,fig.pos='H', fig.cap="Shannon diversity Index, Margalef Richness, Hieps Evenness measured from data collected from DFO's Summer RV survey and has been corrected for catchability. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data."}
...
- There is relative stability or increases in the indicators that reflect the biodiversity of the NAFO 4X area. While improving sampling protocols in the RV surveys have increased Margalef Richness over the last 5 years. Species diversity has been above or at average in the last 5 years. Heips evenness has been relatively consistent over time, but the low values (<0.2) indicates a community with generally fewer, species that are highly abundant. 
...
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=7,fig.pos='H', fig.cap="Total Biomass and guild-level biomasses for fish catagories from DFO's Summer RV survey which have been corrected for catchability. Invertebrate estimates are not corrected for catchability. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data."}
dat.dir<-here::here("data")
...
### Diversity of catches (catch profiles)
This indicator measures the Shannon Diversity (H) of the catches to try to examine the composition of discarded species from a given fishery or group of fisheries. 
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=7,fig.pos='H', fig.cap="Shannon Diversity of species caught in fixed and mobile gear from the cod, haddock, pollock fisheries in NAFO 4X5Y for each quarter. Dark Blue line  is the annual mean +/- se using a LOESS regression model. "}
dat.dir<-here::here("data")
...
```
- Mobile gear cod, haddock, pollock fishery for NAFO 4X5Y show a increasing biodiversity of catches since 2013 for all quarters. This suggests that catches may be becoming less targeted. 

- Fixed gear cod, haddock, pollock fishery show are more stable annual trend. 

### Observer coverage and observed vessels
This indicator attempts to examine the at sea observer coverage for fisheries over time and examine the proportion of vessels that are active in each year that were observed.
```{r fig.align="center", echo = FALSE,fig.width = 10, fig.height=7,fig.pos='H', fig.cap="Observer coverage (%)  and observed vessels (%) by gear type for the cod, haddock, pollack fishery in NAFO 4X5Y. Horizontal lines representent the mean +/- 1 sd. Purple, shaded areas are the latest 5 years of available data."}
dat.dir<-here::here("data")
...
(pq1|pq2)/(pr1|pr2)
```
- Observer coverage and vessel coverage for the fixed gear cod, haddock, pollock fishery in NAFO 4X5Y has been at 0% coverage for the last 5 years, while mobile gear observer and vessel coverage has been high in the most recent year. 
...
```{r fig.align="center", echo = FALSE,fig.width = 10,fig.pos='H', fig.cap="Landed value of species categories since 1990 for NAFO 4X. Area in purple is preliminary data and subject to change." }
...
pc1<- MAReco::plot_multiple_indicators(Landings_value_4X, YEAR, Landed_Value_CAD, Species_Category, y_label= "Landed Value (CAD)", legend_labels=c("Groundfish", "Mollusc and Crustacean", "Pelagic and Estuarial") )
...
- In the last 5 years, the landed value of mollusc and crustacean catches in the NAFO 4X area have fluctuated while still remaining high. The landed value of groundfish catches have remained stable in the area, while pelagic and estuarial catches have been declining slightly over the last 5 years.
